#!/usr/bin/env python3
"""Validate Seedance 2.0 step-one segment plans."""
...
def validate(
    ...
    for segment in segments:
        ...
        if not duration_match:
            errors.append(f"{label}无法解析时长；应写成“13.5 秒”。")
            continue
        ...
        if duration < duration_min or duration > duration_max:
            errors.append(
                ...
                f"{duration_min:g}–{duration_max:g} 秒之间。"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--outline-min", type=int, default=180, help="非对白叙述最少汉字数")
    parser.add_argument("--outline-max", type=int, default=300, help="非对白叙述最多汉字数")
    parser.add_argument(
        "--seconds-per-char",
        type=float,
        default=0.3,
