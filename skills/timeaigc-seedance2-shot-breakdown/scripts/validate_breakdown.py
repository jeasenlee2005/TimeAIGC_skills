#!/usr/bin/env python3
"""Validate structural and timing constraints in Seedance 2.0 shot breakdowns."""
...
def validate(
    ...
    for segment in segments:
        ...
        if not 4 <= len(segment.shots) <= 9:
            errors.append(f"{label}包含 {len(segment.shots)} 个镜头；应为 4–9 个。")
        ...
        if segment_end < duration_min - tolerance or segment_end > duration_max + tolerance:
            errors.append(
                f"{label}时间轴结束于 {segment_end:g} 秒；"
                f"应在 {duration_min:g}–{duration_max:g} 秒之间。"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="检查 Seedance 2.0 分镜的字段、时间轴、镜头数与描述字数。"
    )
    parser.add_argument("input", nargs="?", default="-", help="UTF-8 Markdown 文件；默认从 stdin 读取")
    parser.add_argument("--duration-min", type=float, default=13.0, help="片段最短秒数")
    parser.add_argument("--duration-max", type=float, default=14.0, help="片段最长秒数")
    parser.add_argument("--visual-min", type=int, default=110, help="单镜头视觉描述最少中文字符数")
    parser.add_argument("--visual-max", type=int, default=160, help="单镜头视觉描述最多中文字符数")
